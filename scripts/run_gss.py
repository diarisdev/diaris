...
try:
    ...
    print("❌ Lhotse package is not installed! GSS requires Lhotse.")
    print("Please run: pip install lhotse")
    sys.exit(1)
    ...
    for ch1_path in audio_files:
        ...
        for ch in range(1, 5):
            ...
            if not ch_file.exists():
                print(f"⚠️ Channel file {ch_file} is missing, skipping this channel.")
                continue
        ...
        print(f"📼 Creating Recording manifest for {sess_arr} ({len(sources)} channels, {info.duration/60:.2f} mins)...")
        ...
        if not trans_file.exists():
            print(f"⚠️ Transcription file {trans_file} not found, skipping supervisions for {sess_arr}.")
            continue
            
        with open(trans_file, 'r', encoding='utf-8') as f:
            trans_data = json.load(f)
            
        print(f"📝 Loading supervisions from {trans_file.name}...")
        seg_idx = 0
