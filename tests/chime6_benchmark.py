...
def get_audio_paths(audio_dir, session_id, mode, array="U01", enhance="none", worn_speaker=None):
    ...
    if mode == "worn":
        ...
        if enhance == "gss":
            # Try to look for pre-enhanced GSS files: e.g. S01_U01.GSS.wav or S01_GSS.wav
            for arr in arrays:
                candidates = [
...
def run_chime6_session(audio_dir, transcript_path, mode, array="U01", enhance="none",
                      ...
                      worn_speaker=None, log_file=None):
    """
    ...
    if segmentation == "streaming":
        ...
        print("DEBUG: Checking ref_segments:")
        for idx, s in enumerate(ref_segments):
            if not isinstance(s, dict):
                print(f"DEBUG: Index {idx} is NOT a dict: {type(s)} - {s}")
            else:
                print(f"DEBUG: Index {idx} keys = {list(s.keys())} - {s}")
                
        print("DEBUG: Checking clean_hyp_segments:")
        for idx, s in enumerate(clean_hyp_segments):
            if not isinstance(s, dict):
                print(f"DEBUG: Index {idx} is NOT a dict: {type(s)} - {s}")
            else:
                print(f"DEBUG: Index {idx} keys = {list(s.keys())} - {s}")
        ...
        filtered_refs = [r for r in ref_diar if r["start"] >= warmup_s]
        filtered_hyps = [h for h in hyp_diar if h["start"] >= warmup_s]
